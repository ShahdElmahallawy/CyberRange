...
func SubmitFlag(c echo.Context) error {
    ...
    var count int
    err := db.DB.QueryRow("SELECT COUNT(*) FROM ctf_solves WHERE challenge_name = $1 AND name = $2", challenge, utils.GetName(token)).Scan(&count)
    if err != nil {
        fmt.Println(err)
        return c.JSON(500, map[string]string{"error": "Failed to check flag submission status"})
    }

    if count > 0 {
        return c.JSON(400, map[string]string{"error": "Flag for challenge '" + challenge + "' has already been submitted"})
    }
