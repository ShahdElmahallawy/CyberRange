...
func UpdateAccount(c echo.Context) error {
	...
	if err != nil {
		fmt.Println("Old password mismatch")
	...
	if req.Name != "" {
		var count int
		err = db.DB.QueryRow("SELECT COUNT(*) FROM users WHERE name = $1", req.Name).Scan(&count)
		if err != nil {
			fmt.Println(err)
			return c.JSON(500, map[string]string{"error": "Failed to check name availability"})
		}

		if count > 0 {
			return c.JSON(400, map[string]string{"error": "This name is already taken"})
		}
	...
	return c.JSON(200, map[string]string{"message": "Account updated successfully!"})
}
