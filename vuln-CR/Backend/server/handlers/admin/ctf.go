...
func AddChallenge(c echo.Context) error {
    ...
    if err != nil {
        return c.JSON(400, map[string]string{"error": "Points must be a valid integer"})
    }

    // Validate points is not negative 
    if points < 0 {
        return c.JSON(400, map[string]string{"error": "Points value out of allowed range"})
    }
    ...
    // Join attachments into a single string
    attachmentsStr := strings.Join(attachments, ",")

    // Add the challenge to the database
    err = db.AddChallenge(name, sanitizedDescription, difficulty, flag, strconv.Itoa(points), hint, category, attachmentsStr)
    if err != nil {
        fmt.Println(err)
