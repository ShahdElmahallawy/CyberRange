...
import DOMPurify from "dompurify"; // Ensure you have this library installed
...
// Editted the function bro to handle the xss
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (data.scoreboard && data.scoreboard.length > 0) {
        // Sanitize scoreboard data if rendering raw HTML
        scoreboard.value = data.scoreboard.map((entry) => ({
          ...entry,
          name: DOMPurify.sanitize(entry.name),
          score: DOMPurify.sanitize(entry.score.toString()), // Example: sanitize all fields
        }));
      ...
      if (data.message) {
        ...
        message.value = DOMPurify.sanitize(data.message);
      }
    ...
    alert("An error occurred while fetching the scoreboard");
  } finally {
    loading.value = false;
  }
}
